 ...
 *
 * Purpose:  Multiplicacion matriz vector paralela
 * con MPI para ejecutar en un cluster.
 *
 * Compile:  mpicc -g -Wall -o mpi_mat_vect_mult mpi_mat_vect_mult.c
 * Run:      mpiexec -n 4 ./mpi_mat_vect_mult
 */
...
void Allocate_arrays(double** local_A_pp, double** local_x_pp, 
      ...
      MPI_Comm comm);
void Generate_matrix(double local_A[], int m, int local_m, 
      int n, int my_rank, MPI_Comm comm);
void Generate_vector(double local_vec[], int n, int local_n, 
...
int main(void) {
   ...
   Generate_matrix(local_A, m, local_m, n, my_rank, comm);
#  ifdef DEBUG
   Print_matrix("A", local_A, m, local_m, n, my_rank, comm);
#  endif
   
   Generate_vector(local_x, n, local_n, my_rank, comm);
#  ifdef DEBUG
...
void Get_dims(
   ...
   /* Hardcoding dimensions evenly divisible by 4 (the number of processes) */
   *m_p = 8;
   *n_p = 8;
...
void Generate_matrix(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, "Generate_matrix",
            "Can't allocate temporary matrix", comm);
            
      // Automatically generate entries instead of asking for input
      for (i = 0; i < m; i++)
         for (j = 0; j < n; j++)
            A[i*n+j] = (double)(i + j);
...
void Generate_vector(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, "Generate_vector",
            "Can't allocate temporary vector", comm);
            
      // Automatically generate vector components instead of asking for input
      for (i = 0; i < n; i++)
         vec[i] = 1.0; 
